...
public abstract class DeviceWaitingSearch extends Thread {
    ...
    public DeviceWaitingSearch( String name, String room) {
        deviceName = name;
        deviceRoom = room;
    }
        ...
        try {
            ...
            while (true) {
                ...
                if (verifySearchData(pack)) {
                    ...
                    System.out.println(deviceName +" : 给主机回复信息");
                    socket.send(sendPack);
                    System.out.println(deviceName +" : 等待主机接收确认");
                    socket.setSoTimeout(RECEIVE_TIME_OUT);
                    try {
                        socket.receive(pack);
                        if (verifyCheckData(pack)) {
                            System.out.println(deviceName +" - 确认成功");
    ...
    private boolean verifyCheckData(DatagramPacket pack) {
        ...
        try {
            ...
            ip = new String(data, offset, pack.getLength() - offset, "UTF-8");
            System.out.println(deviceName +" : ip from host =" + ip);
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
    ...
    private String getOwnWifiIP() {
        String ipInt = "127.0.0.1";
        try {
            InetAddress addr = InetAddress.getLocalHost();
            ipInt=addr.getHostAddress();//获得本机IP
